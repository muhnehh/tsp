...
This repository provides a concise yet comprehensive baseline for solving the **Traveling Salesman Problem (TSP)** using deterministic **hill climbing with random restarts** and **2-opt / insertion neighborhoods**. The implementation adheres to the **TSPLIB95 Euclidean convention** and scales efficiently via **sampled neighborhood exploration** for large problem sizes.

---

## Abstract

The Traveling Salesman Problem (TSP) seeks the shortest possible route visiting each city exactly once and returning to the start. As an NP-hard problem, exact algorithms become impractical for large instances. This work implements and analyzes three hill-climbing heuristics—**2-city swap**, **2-opt edge reversal**, and **1-city insertion**—using **nearest-neighbor initialization** and **random restarts**. The results demonstrate that the 2-opt heuristic consistently dominates, achieving near-optimal tours (as close as 0.7% from the optimum) while maintaining scalability through sampled neighborhoods for instances with \( n \ge 150 \).
...
\]
representing a cyclic permutation of all nodes. Since rotation or reversal does not change the geometry of the tour, the number of unique states is:
\[
\frac{(n - 1)!}{2}.
\]
...
\]
where \( d(p, q) = \lfloor \sqrt{(x_p - x_q)^2 + (y_p - y_q)^2} \rceil \) follows the TSPLIB rounding rule.

### 1.3 Initialization — Nearest Neighbor
Starting from a random city, each step appends the closest unvisited city until all are visited. This greedy seed construction runs in \( O(n^2) \) time.

### 1.4 Neighborhood Structures

1. **Swap (i, j):** exchange two cities. Count: \( \binom{n}{2} \).  
2. **2-opt:** reverse the path segment between two edges. Count: \( \binom{n}{2} - n \).  
3. **Insertion:** remove a city and reinsert it elsewhere. Count: \( n \times (n-1) \).  

For \( n < 150 \), all neighbors are evaluated exhaustively. For \( n \ge 150 \), sampled neighborhoods (1000 random trials) are used.
...
Six standard **EUC\_2D** benchmarks from **TSPLIB95** were used:
